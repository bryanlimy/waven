...
def makeGaborFilter(
    i, j, angle, sigma, phase, f=0.4, lx=54, ly=135, plot=False, freq=True
):
    backgrd = np.zeros((lx, ly))
    if freq:
        gk = gabor_kernel(
            frequency=f, theta=angle, sigma_x=sigma, sigma_y=sigma, offset=phase
        )
    else:
        gk = gabor_kernel(
            frequency=(-0.016 * sigma) + 0.148,
            theta=angle,
            sigma_x=sigma,
            sigma_y=sigma,
            offset=phase,
...
def makeGaborFilter3D(
    i, j, angle, sigma, tp_w, f=0.4, lx=54, ly=135, alpha1=0, alpha2=np.pi / 4
):
    ...
    f3d = np.array(
        [
            makeGaborFilter(i, j, angle, sigma, phase, f=f, lx=lx, ly=ly)
            for phase in phases
        ]
    )
                ...
                for s in sigmas:
                    for f in frequencies:
                        for o in offsets:
                            library.append(
                                makeGaborFilter(
                                    x, y, t, s, o, f, lx=lx, ly=ly, freq=True
                                )
                            )

    library = np.array(library)
    return library.reshape(
        (
            lx,
            ly,
            thetas.shape[0],
        ...
        for y in ys:
            for t in thetas:
                for s in sigmas:
                    for o in offsets:
                        library.append(
                            makeGaborFilter(x, y, t, s, o, f, lx=lx, ly=ly, freq=freq)
                        )

    library = np.array(library)
    return library.reshape(
        (lx, ly, thetas.shape[0], sigmas.shape[0], offsets.shape[0], -1)
    )
...
def makeFilterLibrary3D(
    xs, ys, thetas, sigmas, offsets, f, tp_w, alpha1, alpha2, filename
):
    # library=[]
    lx = xs.shape[0]
    ly = ys.shape[0]
    fp = np.zeros(
        shape=(lx, ly, thetas.shape[0], sigmas.shape[0], tp_w, ly, lx), dtype="float16"
    )
    print(fp.shape)
    i = 0
        ...
        for y in ys:
            for i, t in enumerate(thetas):
                for j, s in enumerate(sigmas):
                    l = makeGaborFilter3D(
                        x, y, t, s, tp_w, f, lx=lx, ly=ly, alpha1=alpha1, alpha2=alpha2
                    )
                    # print(l.shape)
                    fp[x, y, i, j] = l
...
def downsample_video_binary(
    ...
    while ret:
        ...
        try:
            ...
            video_bin = skimage.transform.resize(
                video_bin, (chunk_size, shape[0], shape[1])
            )  # 137
...
def downsample_video_uint(path, shape=(54, 135), chunk_size=1000):
    ...
    for i in range(nb_chunks):
        print(i)
        video_bin = skimage.transform.resize(
            video[i * chunk_size : (i + 1) * chunk_size],
            (chunk_size, shape[0], shape[1]),
        )  # 137
