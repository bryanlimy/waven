...
def Decay(t, tau, t0):
    ...
    return 1.0 / tau * np.exp(-t / tau) * 0.5 * (np.sign(t - t0) + 1.0)
...
def orientation_correction_for_stretches(visual_coverage, nx, ny, omax):
    ...
    corrected_ori[np.asarray(corrected_ori < 0).nonzero()[0]] = (
...
def PearsonCorrelationPinkNoise(
    ...
    omax_corr = orientation_correction_for_stretches(
        visual_coverage, nx, ny, omax * 22.5
    )
    ...
    if plotting:
        ...
        if np.sum(fil) == 0:
            ...
            plt.rcParams["axes.facecolor"] = "none"
            plt.scatter(
                neuron_pos[:, 1], neuron_pos[:, 0], s=5, c=xmax_corr, cmap="jet"
            )
            plt.colorbar()
            ...
            plt.rcParams["axes.facecolor"] = "none"
            plt.scatter(
                neuron_pos[:, 1], neuron_pos[:, 0], s=5, c=ymax_corr, cmap="jet"
            )
            plt.colorbar()
            ...
            plt.rcParams["axes.facecolor"] = "none"
            plt.scatter(
                neuron_pos[:, 1], neuron_pos[:, 0], s=5, c=smax_corr, cmap="coolwarm"
            )
            plt.colorbar()
            ...
            plt.figure()
            plt.scatter(
                neuron_pos[fil, 1], neuron_pos[fil, 0], s=5, c=xmax[fil], cmap="jet_r"
            )
            plt.colorbar()
            plt.title("xmax")
            ...
            plt.figure()
            plt.scatter(
                neuron_pos[fil, 1], neuron_pos[fil, 0], s=5, c=ymax[fil], cmap="jet_r"
            )
            plt.colorbar()
            plt.title("ymax")
            ...
            plt.figure()
            plt.scatter(
                neuron_pos[fil, 1], neuron_pos[fil, 0], s=5, c=omax[fil], cmap="jet_r"
            )
            plt.colorbar()
    torch.cuda.empty_cache()
...
def repetability_trial(resps_all, neuron_pos):
    ...
    plt.figure()
    plt.scatter(
        neuron_pos[:, 1], neuron_pos[:, 0], s=5, vmax=1, c=respcorrs, cmap="Greys"
    )
    plt.colorbar()
...
def repetability_trial2(resps_all, neuron_pos):
    ...
    for i in range(n_cell):
        ...
        for t in range(resps_all.shape[0]):
            exclude = [t]
            meanresp = np.mean(
                resps_all[~np.isin(np.arange(len(resps_all)), exclude), :, i], axis=0
            )
            ct = ct + 1
...
def preferedDirection(videodata, spks, x, y, ori, w_i, w_r, window_size=5):
    ...
    pref_dir = np.mean(
        (np.mean(spks[:, :, 9275], axis=0) > 0) * angle * (abs(angle) >= ori - 10)
    )  # to account for the measured error
...
def predictSparseNoise(resp, stim, rfs, tp, save=False):
    ...
    if save:
        tifffile.imwrite(
            "/media/sophie/Expansion1/UCL/datatest/SP045/2023-10-04/3/tp3000reconstructed.tif",
            img.reshape(54, 135),
        )
...
def predictPinkNoise(maxes, vis_n, spks, rfs, tp, L, videodata, dt=50, save=False):
    ...
    try:
        ...
        rfs_model = np.swapaxes(rfs[0], 1, 2) * rfs_model.reshape(
            spks.shape[-1], 27, 11, 8, 4
        )
    ...
    if save:
        ...
        tifffile.imwrite(
            "/media/sophie/Expansion1/UCL/datatest/SP045/2023-10-04/3/tp3520videodata.tif",
    ...
    if save:
        tifffile.imwrite(
            "/media/sophie/Expansion1/UCL/datatest/SP045/2023-10-04/3/tp3520reconstructed_1wavelet.tif",
...
def DirectionSelectivity(x, y, o, s, w_i_downsampled, w_r_downsampled):
    ...
    tan = np.diff(
        signal.sosfilt(
            sos, w_i_downsampled[:, x, y, o, s] / w_r_downsampled[:, x, y, o, s]
        ),
        append=0,
    )
...
def DirectionSelectivityPlot(
    idx, spks, x, y, o, s, w_i_downsampled, w_r_downsampled, plotting=False
):
...
def SinCosPlot(idx, spks, x, y, o, s, w_i_downsampled, w_r_downsampled, ncut):
    ...
    data["z_past"] = Y_past
    cuts = pd.DataFrame(
        {str(feature) + "Bin": pd.cut(data[feature], nCut) for feature in [0, 1]}
    )
    ...
    dx = np.array(
        [
            0.5 * (means["z"].columns.array[i].left + means["z"].columns.array[i].right)
            for i in range(nCut)
        ]
    )
    dy = np.array(
        [
            0.5 * (means["z"].index.array[i].left + means["z"].index.array[i].right)
            for i in range(nCut)
        ]
    )
    theta1 = (
        np.arctan(
            dy[np.where(z == np.max(np.nan_to_num(z)))[0][0]]
            / dx[np.where(z == np.max(np.nan_to_num(z)))[1][0]]
        )
        * 180
        / np.pi
    )
    ...
    dx = np.array(
        [
            0.5
            * (
                means["z_past"].columns.array[i].left
                + means["z"].columns.array[i].right
            )
            for i in range(nCut)
        ]
    )
    dy = np.array(
        [
            0.5
            * (means["z_past"].index.array[i].left + means["z"].index.array[i].right)
            for i in range(nCut)
        ]
    )
    theta2 = (
        np.arctan(
            dy[np.where(z2 == np.max(np.nan_to_num(z2)))[0][0]]
            / dx[np.where(z2 == np.max(np.nan_to_num(z2)))[1][0]]
        )
        * 180
        / np.pi
    )
...
def interpolateData3d(z, dx, dy, dp, ncut, smooth=True):
    ...
    interp_grid = griddata(
        points, np.nan_to_num(values), (x_grid, y_grid, p_grid), method="nearest"
    )
    if smooth:
...
def interpolateData(z, dx, dy, ncut, smooth=True):
    ...
    # generate interpolated grid data
    interp_grid = griddata(
        points, np.nan_to_num(values), (x_grid, y_grid), method="nearest"
    )
    if smooth:
...
def SinCosPlot2(idx, spk, w_i, w_r, dphi, noise, ncut, smoothing_size, plotting=True):
    ...
    nCut = ncut
    data = pd.DataFrame(
        np.concatenate([cos, sin, dphi, noise], axis=1)
    )  # , columns=['sin', 'cos', 'sig'])
    data["z"] = Y
    data["z_past"] = Y_past
    cuts = pd.DataFrame(
        {str(feature) + "Bin": pd.cut(data[feature], nCut) for feature in [0, 1, 2, 3]}
    )
    # print('at first cuts are pandas intervalindex.')
    # print(cuts.head())
    # print(cuts.info())
    ...
    dy = np.array(
        [
            0.5
            * (
                means["z"].index.to_numpy()[i][0].left
                + means["z"].index.to_numpy()[i][0].right
            )
            for i in np.arange(0, nCut**3, nCut**2)
        ]
    )
    dp = np.array(
        [
            0.5
            * (
                means["z"].index.to_numpy()[i][1].left
                + means["z"].index.to_numpy()[i][1].right
            )
            for i in np.arange(0, nCut**2, nCut)
        ]
    )
    dn = np.array(
        [
            0.5
            * (
                means["z"].index.to_numpy()[i][2].left
                + means["z"].index.to_numpy()[i][2].right
            )
            for i in np.arange(0, nCut)
        ]
    )
...
def diffSinCosPlot(idx, spks, x, y, o, s, w_i_downsampled, w_r_downsampled):
    ...
    sin_sos = signal.sosfilt(
        sos, np.diff(w_i_downsampled[:, x, y, o, s], append=0).reshape(-1, 1)
    )
    cos_sos = signal.sosfilt(
        sos, np.diff(w_r_downsampled[:, x, y, o, s], append=0).reshape(-1, 1)
    )
    ...
    nCut = 30
    data = pd.DataFrame(
        np.concatenate([cos_sos, sin_sos], axis=1)
    )  # , columns=['sin', 'cos', 'sig'])
    data["z"] = Y
    cuts = pd.DataFrame(
        {str(feature) + "Bin": pd.cut(data[feature], nCut) for feature in [0, 1]}
    ...
    plt.clf()
    sns.heatmap(
        means["z"].interpolate(method="linear", limit=2, limit_direction="both"),
        cmap="coolwarm",
    )
    plt.title("Means of spks vs Features cos and sin")
    plt.tight_layout()
    ...
    plt.figure()
    sns.jointplot(
        x=w_i_downsampled[:, x, y, o, s].reshape(-1, 1)[:, 0],
        y=w_r_downsampled[:, x, y, o, s].reshape(-1, 1)[:, 0],
        kind="kde",
        fill=True,
    )
    ...
    c = np.array(
        [
            (
                torch.sum(
                    a[ran:].reshape(1, -1)
                    @ b[ran - dt1 : b.shape[0] - dt1].reshape(b.shape[0] - ran, -1),
                    axis=0,
                )
                / torch.sum(a[ran:])
            )
...
def spikeTrig(spk, w_i, w_r, w_c, ran):
    ...
    mu_sin = np.array(
        [
            np.sum(spk[ran:].T * w_i[ran - dt1 : w_i.shape[0] - dt1])
            / np.sum(spk[ran:])
            for dt1 in range(ran)[::-1]
        ]
    )
    ...
    mu_cos = np.array(
        [
            np.sum(spk[ran:].T * w_r[ran - dt1 : w_r.shape[0] - dt1])
            / np.sum(spk[ran:])
            for dt1 in range(ran)[::-1]
        ]
    )
    ...
    mu_complex = np.array(
        [
            np.sum(spk[ran:].T * w_c[ran - dt1 : w_c.shape[0] - dt1])
            / np.sum(spk[ran:])
            for dt1 in range(ran)[::-1]
        ]
    )
...
def compute_stc(a, b, mu_b, ran, dt1, dt2):
    ...
    c = (
        (
            torch.matmul(
                a[ran - dt1 : a.shape[0] - dt1].reshape(1, -1),
                (
                    b[ran - dt1 : b.shape[0] - dt1] - mu_b[mu_b.shape[0] - 1 - dt1]
                ).reshape(b.shape[0] - ran, -1),
            ).T
            @ torch.matmul(
                a[ran - dt1 : a.shape[0] - dt1].reshape(1, -1),
                (
                    b[ran - dt2 : b.shape[0] - dt2] - mu_b[mu_b.shape[0] - 1 - dt2]
                ).reshape(b.shape[0] - ran, -1),
            )
            / torch.sum(a[ran - dt1 : a.shape[0] - dt1])
        )
        .detach()
        .cpu()
        .numpy()
...
def CovspikeTrig(spk, w, mu, ran):
    Css = np.zeros(
        (np.max(abs(np.array(ran))), 54 * 135, np.max(abs(np.array(ran))), 54 * 135),
        dtype="float16",
    )
    ...
    for dt1 in range(ran):
        for dt2 in range(ran):
            print(dt1, dt2)
            Css[dt1, :, dt2, :] = compute_stc(spk, w, mu, ran, dt1, dt2).astype(
                "float16"
            )
            # Css[dt1, dt2] = (np.sum(
...
def CovspikeTrigC(spk, w_i, w_r, mu_i, mu_r, ran):
    ...
    for dt1 in range(ran):
        for dt2 in range(ran):
            Css[dt1, dt2] = np.sum(
                spk[ran:]
                * (w_i[ran - dt1 : w_i.shape[0] - dt1] - mu_i[dt1])
                * (w_r[ran - dt2 : w_r.shape[0] - dt2] - mu_r[dt2]),
                axis=0,
            ) / np.sum(spk[ran:])
...
def getSVDPolar(
    idx, spk, ncut, args, plotting=False, more_smooth=False, smoothing_size=5
):
    ...
    f = interpolate.LinearNDInterpolator(
        np.stack((rho.flatten(), phi.flatten(), dd.flatten())).T, zz.flatten().T
    )
    if more_smooth:
        s = ncut
        ...
        # generate interpolated grid data
        interp_grid = griddata(
            points, np.nan_to_num(values), (xx, yy, tt), method="nearest"
        )
        # plt.figure()
        ...
        if plotting:
            ...
            plt.pcolormesh(
                rhospace,
                phispace,
                np.nanmean(interp_grid, axis=2)[:-1, :-1],
                cmap="coolwarm",
            )
        ...
        f = interpolate.LinearNDInterpolator(
            np.stack((xx.flatten(), yy.flatten(), tt.flatten())).T,
...
def getNonLinearModel(idx, spks, x, y, o, s, w_i_downsampled, w_r_downsampled, ncut):
    ...
    rho, phi, z, theta1, dx, dy = SinCosPlot(
        idx, spks[:, 6:, :], x, y, o, s, w_i_downsampled[6:], w_r_downsampled[6:], ncut
    )
    from scipy.stats import binned_statistic

    mean_rho = binned_statistic(
        np.nan_to_num(rho.flatten()),
        np.nan_to_num(z.flatten()),
        statistic="max",
        bins=10,
    )
    mean_phi = binned_statistic(
        np.nan_to_num(phi.flatten()),
        np.nan_to_num(z.flatten()),
        statistic="max",
        bins=10,
    )
...
def getNonLinearModel2(
    ...
    rr, pp, dd, nn, zz, hanz, dx, dy, dp, dn = SinCosPlot2(
        idx, spk, w_i, w_r, dphi, noise, ncut, smoothing_size, plotting=plotting
    )
...
def fitnonlin(X1, y_train, func):
    ...
    fittedParameters, pcov = curve_fit(
        func, X1, y_train, geneticParameters, method="trf"
    )
...
def PlotR2scoreAnalysis(path, neuron_pos, respcorr):
    ...
    plt.rcParams["axes.facecolor"] = "none"
    plt.scatter(
        neuron_pos[:, 1], neuron_pos[:, 0], s=5, c=r2, vmax=0.1, vmin=0, cmap="Greys"
    )
    plt.colorbar()
    plt.title("R2")
    plt.figure()
...
def calculate_spike_triggered_covariance(spikes, stimulus, tau):
    ...
    for t in range(tau, T):
        if spikes[t] != 1:
            segment = spikes[t] * stimulus[:, t - tau : t].reshape(
                -1
            )  # Transformer en vecteur de taille (X*tau,)
    ...
    for start_i in range(0, X * tau, slice_size):
        ...
        for start_j in range(start_i, X * tau, slice_size):
            ...
            covariance_matrix[start_i:end_i, start_j:end_j] = (
                partial_cov.detach().cpu().numpy()
            )
            if start_i != start_j:
                covariance_matrix[start_j:end_j, start_i:end_i] = (
                    partial_cov.t().detach().cpu().numpy()
                )
...
def create_fake_cell(
    ...
    interp_grid = (
        (
            (1 + np.cos(phispace + phase)).reshape(-1, 1)
            * sigma_func(rhospace, 100, thresh).reshape(1, -1)
        ).reshape(-1, 1)
        * (2.5 * gaussian(dphispace, phase_t_shift, 150)).reshape(1, -1)
    ).reshape(ncut, ncut, ncut)
...
def PlotSelfCorrelation(w_c_downsampled, neuron_pos, pos_ori, ns=4):
    ...
    for i in range(8):
        for j in range(ns):
            ax[i, j].imshow(
                rfs_idx[:, :, i, j].T, vmin=vmin, vmax=vmax, cmap="coolwarm"
            )
...
def interpolateDatand(z, dx, dy, dp, dx_h, dy_h, dp_h, ncut, smooth=True):
    # from scipy.interpolate import RegularGridInterpolator
    # f= RegularGridInterpolator((dx, dp, dy), z)
    x_grid, y_grid, p_grid, xh_grid, yh_grid, ph_grid = np.meshgrid(
        dx, dy, dp, dx_h, dy_h, dp_h
    )
...
def SinCosPlot3(
    ...
    nCut = ncut
    data = np.concatenate(
        [cos, sin, dphi, sin_h, cos_h, dphi_h], axis=1
    )  # , columns=['sin', 'cos', 'sig'])
...
def getNonLinearModel3(
    ...
    interp_grid, z, dx, dy, dp, dx_h, dy_h, dp_h = SinCosPlot3(
        idx, spk, w_i, w_r, dphi, noise, ncut, smoothing_size, plotting=plotting
    )
    x_grid, y_grid, p_grid, xh_grid, yh_grid, ph_grid = np.meshgrid(
        dx, dy, dp, dx_h, dy_h, dp_h
...
def getmetrics(x, y, n):
    ev = explained_variance_score(
        np.mean(y.reshape(n, 1800), axis=0), x, multioutput="uniform_average"
    )
...
def GetNeuronVisresponse(
    ...
    f, f_h, d, d_h, c, c_h, hmp, hmp_h, plot = getPhiRho(
        y_train[:, :dt1],
        w_i[:dt1],
        w_r[:dt1],
        dphi[:dt1],
    ...
    X1 = np.concatenate(
        [(np.concatenate((pred[:dt1], pred_h[:dt1]), axis=1)) for rep in train_idx]
    )
    ...
    ev = explained_variance_score(
        np.mean(y_train.reshape(len(train_idx), dt1), axis=0),
        res2,
        multioutput="uniform_average",
    )
    feve = FEVE(y_train.reshape(len(train_idx), dt1), res2)
    cc_train = np.corrcoef(np.mean(y_train.reshape(len(train_idx), dt1), axis=0), res2)[
        0
    ][1]
    print(feve)
    print(ev)
    ...
    X1 = np.concatenate(
        [(np.concatenate((pred[:dt1], pred_h[:dt1]), axis=1)) for rep in test_idx]
    )
    X1 = np.nan_to_num(X1)
    print(X1.shape)
    ...
    if double_wavelet_model:
        ...
        res21 = unrectified
        cc1 = np.corrcoef(np.mean(y_test.reshape(len(test_idx), dt1), axis=0), res21)[
            0, 1
        ]
        print(cc1)
        res22 = unrectified2
        cc2 = np.corrcoef(np.mean(y_test.reshape(len(test_idx), dt1), axis=0), res22)[
            0, 1
        ]
        print(cc2)
        if cc1 >= cc2:
    ...
    ev = explained_variance_score(
        np.mean(y_test.reshape(len(test_idx), dt1), axis=0),
        res2,
        multioutput="uniform_average",
    )
    ...
    if lastmin:
        ...
        X1 = np.concatenate(
            [
                (np.concatenate((pred[tp_test:], pred_h[tp_test:]), axis=1))
                for rep in test_idx
            ]
        )
        ...
        print(
            np.mean(y_test.reshape(len(test_idx), dt1 - tp_test), axis=0).shape,
            reslastmin.shape,
        )
        evlastmin = explained_variance_score(
            np.mean(y_test.reshape(len(test_idx), dt1 - tp_test), axis=0),
            reslastmin,
            multioutput="uniform_average",
        )
        fevelastmin = FEVE(y_test.reshape(len(test_idx), dt1 - tp_test), reslastmin)
        cclastmin = np.corrcoef(
            np.mean(y_test.reshape(len(test_idx), dt1 - tp_test), axis=0), reslastmin
        )[0, 1]
    ...
    return (
        res2,
        [feve, ev, cc, cc_train, cclastmin],
        fittedParameters,
        [d, c, hmp, d_h, c_h, hmp_h],
        plot,
        unrectified,
        w,
    )
...
def gaussian_smooth(y, sigma=2):
    ...
    y_smooth = y_smooth[
        int((gaussian_kernel.shape[0] - 1) / 2)
        - 1 : -int((gaussian_kernel.shape[0] - 1) / 2)
        - 1
    ]
...
def approx_Matrix2(X, smoothing_factor=0.75, plotting=False):
    weights, factors = non_negative_parafac(
        X, rank=1, init="random", normalize_factors=False
    )
    U1, U2, U3 = factors
...
def getPhiRho(
    ...
    interp = NearestNDInterpolator(
        np.transpose((xcenters[mask[0]], ycenters[mask[1]], zcenters[mask[2]])), Z[mask]
    )
    indices = np.indices(Z.shape)
    indices = np.stack(
        [xcenters[indices[0]], ycenters[indices[1]], zcenters[indices[2]]]
    )
    ...
    d = zcenters[
        np.argmax(plot[2])
    ]  # zcenters[np.argmax(np.mean(filled_data, axis=(0, 1)))]
    ...
    if plotting:
        ...
        a1_1.set_xticklabels(
            10 * (0.5 * (Ecs[0][1:] + Ecs[0][:-1])).astype(int) / 10, rotation=45
        )
    ...
    mask = np.where(~np.isnan(Z))
    interp_h = NearestNDInterpolator(
        np.transpose((xcenters[mask[0]], ycenters[mask[1]], zcenters[mask[2]])), Z[mask]
    )
    indices = np.indices(Z.shape)
    indices = np.stack(
...
def PredictNeuronsTest(wt_test, spks, idx, ncut, dt1=9000, func=relu):
    ...
    for i in [0, 2]:
        ...
        histo, edges = np.histogram(
            wt_test,
            bins=np.linspace(wt_test.min(), wt_test.max(), ncut + 1),
            density=False,
            weights=Y[:, 0],
        )
        histo_, edges_ = np.histogram(
            wt_test, bins=np.linspace(wt_test.min(), wt_test.max(), ncut + 1)
        )
        H.append(histo)
        H_.append(histo_)
    ...
    # pred_h=w_c
    X1 = np.concatenate([(np.concatenate((pred, pred_h), axis=1)) for rep in train_idx])
...
def split_trials(n_stim, n_rep, n_split=2):
    ...
    for st_idx in range(n_stim):
        ...
        for split_idx in range(n_split):
            split_trials = trial_order[
                split_idx * n_trial_split : (split_idx + 1) * n_trial_split
            ]
...
def getHVA(signMap, neuron_pos, thresh=0.3, sign=1):
    ...
    opening = cv.morphologyEx(
        signMap_binary.astype("uint8"), cv.MORPH_OPEN, kernel, iterations=2
    )
    ...
    markers2 = cv.watershed(
        cv2.merge((sm.astype("uint8"), sm.astype("uint8"), sm.astype("uint8"))), markers
    )
    # sm[markers2 == -1] = [255, 0, 0]
    markers2_neurons = np.array(
        [
            markers2[
                np.maximum(0, int(neuron_pos[i, 1]) - 1),
                np.maximum(0, int(neuron_pos[i, 0]) - 1),
            ]
            for i in range(neuron_pos.shape[0])
        ]
    )  # np.zeros_like(maxes[0, :])
...
def filter_nan_gaussian_conserving2(arr, sigma, mode="reflect"):
    ...
    loss[nan_msk] = 1
    loss = ndimage.gaussian_filter(loss, sigma=sigma, mode=mode, cval=1)

    gauss = arr / (1 - loss)
    gauss[nan_msk] = 0
    gauss = ndimage.gaussian_filter(gauss, sigma=sigma, mode=mode, cval=0)
...
def getSignMap(neuron_pos, maxes, plotting=False):
    ...
    if plotting:
        ...
        plt.imshow(
            signMap_blur,
            vmin=-np.max(signMap_blur),
            vmax=np.max(signMap_blur),
            cmap="coolwarm",
        )
        plt.colorbar()
...
def TwoDimColorMap(X, Y, plotting=False):
    ...
    rgbImage = skimage.transform.resize(
        rgbImage,
        (11, 27),
        mode="edge",
        order=0,
...
def run_Model(
    ...
    for idx in range(spks.shape[2]):
        ...
        w_i = wavelets_i[:, x, y, o, s].reshape(
            -1, 1
        )  # +w_i_downsampled[:, x1, y1, o1, s1]
        w_r = wavelets_r[:, x, y, o, s].reshape(
            -1, 1
        )  # +w_r_downsampled[:, x1, y1, o1, s1]
        ...
        #
        rp_inhib = np.array(
            [cart2pol([w_r_inhib[i]], [w_i_inhib[i]]) for i in range(9000)]
        )
        ...
        vis_resp, a, nonlinparams, rhophiparams, plots, unrectified, w, interp = (
            GetNeuronVisresponse(
                idx,
                w_i,
                w_r,
                w_i_inhib,
...
def run_Full_Model(
    ...
    if memmapping:
        tt = [0, 18000]
        wavelets_i = np.load(wavelet_path + "dwt_videodata2_i.npy", mmap_mode="r")[
            tt[0] : tt[1]
        ]
        wavelets_r = np.load(wavelet_path + "dwt_videodata2_r.npy", mmap_mode="r")[
            tt[0] : tt[1]
        ]
        ...
        wavelets = load_stimulus_simple_cell2(
            wavelet_path, tt=[0, 18000], downsampling=False
        )
    ...
    def findBestPos_profiled(x, y, o, s, nmin=5, plotting=False):
        ...
        wavelets_i = np.load(wavelet_path + "dwt_videodata2_i.npy", mmap_mode="r")[
            tt[0] : nmin * 1800, xt : xt + (2 * w), yt : yt + (2 * w), :, :, :
        ]
        ...
        while div:
            print(x, y)
            wavelets_complex = np.power(wavelets_r[:, x, y], 2) + np.power(
                wavelets_i[:, x, y], 2
            )
            cc_f_1 = (
                torch.corrcoef(
                    torch.Tensor(
                        torch.concatenate(
                            (
                                torch.Tensor(
                                    wavelets_complex.reshape(nmin * 1800, -1).T
                                ),
                                torch.Tensor(spk_train.T),
                            ),
                            axis=0,
                        )
                    ).cuda()
            ...
            if plotting:
                ...
                for i in range(8):
                    ax[i].imshow(
                        cc_f_1[i].T,
                        vmin=-np.max(abs(cc_f_1)),
                        vmax=np.max(abs(cc_f_1)),
                        cmap="coolwarm",
                    )
            ...
            wc = np.sqrt(
                np.power(wavelets_r[:, :, :, o, s, f], 2)
                + np.power(wavelets_i[:, :, :, o, s, f], 2)
            )
            # wc=0
            if r == 1:
                wi = wavelets_i[:, :, :, o, s, f]
            ...
            cc_f_1_xy = (
                torch.corrcoef(
                    torch.Tensor(
                        torch.concatenate(
                            (
                                torch.Tensor(ww.reshape(nmin * 1800, -1).T),
                                torch.Tensor(spk_train.T),
                            ),
                            axis=0,
                        )
    ...
    def findBestPos(x, y, o, s, nmin=5, plotting=False):
        ...
        torch.cuda.empty_cache()
        while div:
            wavelets_complex = np.power(wavelets_r[: nmin * 1800, x, y], 2) + np.power(
                wavelets_i[: nmin * 1800, x, y], 2
            )
            ...
            wc = np.sqrt(
                np.power(
                    wavelets_r[
                        : nmin * 1800,
                        np.maximum(0, x0 - w) : np.maximum(0, x0 - w) + (2 * w),
                        np.maximum(0, y0 - w) : np.maximum(0, y0 - w) + (2 * w),
                        o,
                        s,
                        f,
                    ],
    ...
    for (
        ...
        idxs
    ):  # np.asarray(neuron_pos[:, 1]>600).nonzero()[0]:[1024, 732, 1789, 3279, 614]:#
        gc.collect()
        torch.cuda.empty_cache()
        print(idx)
        x, y, o, s = maxes1[:, idx]
        ...
        if memmapping:
            (x, y, o, s, f) = findBestPos_profiled(
                int(np.round(x)), int(np.round(y)), int(np.round(o)), int(np.round(s))
            )
        ...
        if memmapping:
            ...
            wavelets_i = np.array(wavelets_i)
            wavelets_r = np.load(wavelet_path + "dwt_videodata2_i.npy", mmap_mode="r")[
                tt[0] : n_min * 1800, x, y, :, :, :
            ]
            wavelets_r = np.array(wavelets_r)
            wc = np.sqrt(np.power(wavelets_r, 2) + np.power(wavelets_i, 2))
        else:
            wc = np.sqrt(
                np.power(wavelets_r[: n_min * 1800, x, y, :, :, :], 2)
                + np.power(wavelets_i[: n_min * 1800, x, y, :, :, :], 2)
            )
        ...
        cc_f_1_o = (
            torch.corrcoef(
                torch.Tensor(
                    torch.concatenate(
                        (
                            torch.Tensor(wc.reshape(n_min * 1800, -1).T),
                            torch.Tensor(
                                np.mean(spks[:, : n_min * 1800, idx], axis=0).reshape(
                                    1, -1
                                )
                            ),
                        ),
        ...
        if plotting:
            plt.rcParams.update(
                {"font.size": 8, "svg.fonttype": "none"}  # Texte éditable dans Inkscape
            )

            if memmapping:
                wavelets_i = np.load(
                    wavelet_path + "dwt_videodata2_i.npy", mmap_mode="r"
                )[tt[0] : n_min * 1800, :, :, o, s, f]
                wavelets_i = np.array(wavelets_i)
                wavelets_r = np.load(
                    wavelet_path + "dwt_videodata2_r.npy", mmap_mode="r"
                )[tt[0] : n_min * 1800, :, :, o, s, f]
                wavelets_r = np.array(wavelets_r)
                wc = np.sqrt(np.power(wavelets_r, 2) + np.power(wavelets_i, 2))
            else:
                wc = np.sqrt(
                    np.power(wavelets_r[: n_min * 1800, :, :, o, s, f], 2)
                    + np.power(wavelets_i[: n_min * 1800, :, :, o, s, f], 2)
                )
            cc_f_1_xy = (
                torch.corrcoef(
                    torch.Tensor(
            ...
            azi_val = (
                abs(azi_val - cc_f_1_xy.shape[0]) * (abs(xM - xm) / cc_f_1_xy.shape[0])
            ) + xm
            elev_val = (
                abs(elev_val - cc_f_1_xy.shape[1]) * (abs(yM - ym) / cc_f_1_xy.shape[1])
            ) + ym
            ...
            ax[2].plot(
                np.append(cc_f_1_o[:, s, f], cc_f_1_o[0, s, f]),
                "o-",
                c="k",
            ...
            ax[4].plot(
                frequencies, cc_f_1_o[o, s, :], "o-", c="k", markersize=markersize
            )
            ax[4].set_xticks(
                np.floor(frequencies * np.array([100, 100, 100, 100])).astype(int)
        ...
        if memmapping:
            w_i = np.load(wavelet_path + "dwt_videodata2_i.npy", mmap_mode="r")[
                tt[0] : tt[1], x, y, o, s, f
            ]
            w_i = np.array(w_i).reshape(-1, 1)
            w_r = np.load(wavelet_path + "dwt_videodata2_r.npy", mmap_mode="r")[
                tt[0] : tt[1], x, y, o, s, f
            ]
            w_r = np.array(w_r).reshape(-1, 1)

            w_i_inhib = np.load(wavelet_path + "dwt_videodata2_i.npy")[
                tt[0] : tt[1], x1, y1, o1, s1, f1
            ]
            w_i_inhib = np.array(w_i_inhib).reshape(-1, 1)
            w_r_inhib = np.load(wavelet_path + "dwt_videodata2_r.npy")[
                tt[0] : tt[1], x1, y1, o1, s1, f1
            ]
            w_r_inhib = np.array(w_r_inhib).reshape(-1, 1)
        else:
            w_i = wavelets_i[:, x, y, o, s, f].reshape(
                -1, 1
            )  # +w_i_downsampled[:, x1, y1, o1, s1]
            w_r = wavelets_r[:, x, y, o, s, f].reshape(
                -1, 1
            )  # +w_r_downsampled[:, x1, y1, o1, s1]
        ...
        #
        rp_inhib = np.array(
            [cart2pol([w_r_inhib[i]], [w_i_inhib[i]]) for i in range(tt[1])]
        )
        rho_inhib = rp_inhib[:, 0, 0, 0]
        phi_inhib = rp_inhib[:, 1, 0, 0]
        phi_inhib = np.unwrap(phi_inhib)
        ...
        if plotting:
            ...
            # plt.tight_layout()
            fig.savefig(
                savepath + str(idx) + "new.svg", format="svg", bbox_inches="tight"
            )
    ...
    np.save(
        os.path.join(
            full_path, "RPdp_predictions_8_noneigh_c_smoothpos_" + str(n_min) + ".npy"
        ),
        Predictions,
    )
    np.save(
        os.path.join(
            full_path, "RPdp_nonlinparams_8_noneigh_c_smoothpos_" + str(n_min) + ".npy"
        ),
        nonlinParams,
    )
    np.save(
        os.path.join(
            full_path, "RPdp_rhophiparams_8_noneigh_c_smoothpos_" + str(n_min) + ".npy"
        ),
        RhoPhiParams,
    )
    np.save(
        os.path.join(
            full_path, "RPdp_metrics_8_noneigh_c_smoothpos_" + str(n_min) + ".npy"
        ),
        M,
    )
    np.save(
        os.path.join(full_path, "RPdp_os_8_noneigh_c_smoothpos_" + str(n_min) + ".npy"),
        OS,
    )
    np.save(
        os.path.join(
            full_path, "RPdp_params_8_noneigh_c_smoothpos_" + str(n_min) + ".npy"
        ),
        np.array(Params),
    )
    with open(
        os.path.join(full_path, "interpolators_" + str(n_min) + ".pkl"), "wb"
    ) as f:
