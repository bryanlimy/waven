...
def load_wavelets(pathdir, nx, ny, wavelets_r, wavelets_i, direction=False):
    ...
    pn_wavelets = np.array(
        [
            pn_wavelets[i] / (sigma + np.sum(pn_wavelets[i], axis=0))
            for i in range(pn_wavelets.shape[0])
        ]
    )
    ...
    if direction:
        ...
        sh[0] = sh[0] + 1
        phase = np.insert(
            np.arctan(wavelets_i / wavelets_r), [0], np.zeros((ny, nx, 3, 9)), 0
        ).reshape(sh)
...
def load_stimulus(pathdir, wavelets_r, wavelets_i, nx=161, ny=60):
    ...
    if not direct:
        ...
        wavelets = wavelets.reshape(
            (
                wavelets.shape[0],
                wavelets.shape[1] * wavelets.shape[2] * wavelets.shape[3],
            )
        )
    elif direct:
        w_r = skimage.transform.resize(
            wavelets_r.reshape((-1, ny, nx, 3, 9)), (wavelets_r.shape[0], 8, 20, 3, 9)
        )
        w_i = skimage.transform.resize(
            wavelets_i.reshape((-1, ny, nx, 3, 9)), (wavelets_i.shape[0], 8, 20, 3, 9)
        )
        ...
        pn_wavelets_fwd = pn_wavelets * np.insert(
            np.clip(phase_diff, 0, None), [0], np.zeros((8, 20, 3, 9)), 0
        ).reshape(9000, 8, 20, 3, 9)
...
def load_stimulus_simple_cell(
    ...
    if downsampling:
        wavelets_r = skimage.transform.resize(
            wavelets_r, (wavelets_r.shape[0], nx, ny, no, ns, nf), anti_aliasing=True
        )
        wavelets_r = np.swapaxes(wavelets_r, 2, 1)

        wavelets_i = skimage.transform.resize(
            wavelets_i, (wavelets_i.shape[0], nx, ny, no, ns, nf), anti_aliasing=True
        )
...
def load_stimulus_simple_cell2_i(
    path="/media/sophie/Expansion1/UCL/datatest/", tt=[0, 9000], downsampling=False
):

    wavelets_i = np.load(path + "dwt_videodata2_i.npy")[
        tt[0] : tt[1]
    ]  # ,  mmap_mode='c')
    if downsampling:
        ...
        # wavelets_i=np.swapaxes(wavelets_i, 2,1)
        wavelets_i = skimage.transform.resize(
            wavelets_i[:, :, :, :, 2, :], (9000, 27, 11, 8, 4), anti_aliasing=True
        )
...
def load_stimulus_simple_cell2_r(
    ...
    wavelets_r = np.load(path + "dwt_videodata2_r.npy")[
        tt[0] : tt[1]
    ]  # , mmap_mode='c')
    W_R = []
    if downsampling:
        ...
        #     print(i)
        wavelets_r = skimage.transform.resize(
            wavelets_r[:, :, :, :, 2, :], (tt, 27, 11, 8, 4), anti_aliasing=True
        )
...
def loadExperiment(
    dirs, exp_info, pathdir, block_end, n_planes, n_repeat=6, n_frames=9000
):
    ...
    frame_times = frame_times[
        : frame_times.shape[0] - (frame_times.shape[0] % n_planes)
    ]  # make it dividable by n_plane
    frame_times = frame_times.reshape((-1, n_planes))
    # frame_times=np.mean(frame_times, axis=1)

    neuron_pos = np.concatenate(
        [
            np.asarray(
                [
                    sta["med"]
    ...
    for plane in range(n_planes):
        print("plane :", plane)
        F = np.load(dirs[0] + exp_path + "/suite2p/plane%d/F.npy" % plane)[
            :, block_end:
        ][
            np.load(dirs[0] + exp_path + "/suite2p/plane%d/iscell.npy" % plane)[
                :, 0
            ].astype(bool)
        ]
        Fneu = np.load(dirs[0] + exp_path + "/suite2p/plane%d/Fneu.npy" % plane)[
            :, block_end:
        ][
            np.load(dirs[0] + exp_path + "/suite2p/plane%d/iscell.npy" % plane)[
                :, 0
        ...
        spks_rt = np.array(
            [spks_rt[:, i] - np.min(spks_rt, axis=1) for i in range(spks_rt.shape[1])]
        ).T
        # print(i)
        ...
        try:
            resps_all = utils.interp_event_responses(
                frame_times[:, plane],
                spks_rt,
                events=syncEcho_flip_times,
                window=window,
            ...
            resps_all = utils.interp_event_responses(
                frame_times[: spks_rt.shape[1], plane],
                spks_rt,
                events=syncEcho_flip_times,
                window=window,
                mean_over_window=False,
...
def align_rotary_encoder(
    ...
    syncEcho_flip_times = tl["rawDAQTimestamps"][syncEcho_flip]
    print("syncEcho_flip_times: ", syncEcho_flip_times.shape)
    rotary_encoder_vals = np.clip(
        np.diff(tl["rawDAQData"][:, rotary_encoder_ind].flatten()), -10, 10
    )[syncEcho_flip]
...
def align_datas(
    ...
    if nb_plane != 1:
        ...
        frame_times = frame_times[(frame_times.shape[0] % nb_plane) :].reshape(
            -1, nb_plane
        )
    ...
    while tt:
        try:
            if t == 1:
                print("trial", t)
                trial1 = np.logical_and(
                    frame_times >= syncEcho_flip_times[0],
                ...
                print("trial", t)
                trial1 = np.logical_and(
                    frame_times >= syncEcho_flip_times[Nb_frames * (t - 1)],
                    frame_times < syncEcho_flip_times[Nb_frames * t],
                )
                time_trial1 = frame_times[trial1]
                t = t + 1
            ...
            temp = np.logical_and(
                ...
                frame_times
                < syncEcho_flip_times[
                    np.minimum(Nb_frames * t, syncEcho_flip_times.shape[0] - 1)
                ],
            )
    ...
    for i, trial in enumerate(trials):
        ...
        if exptype == "zebra" or exptype == "sparse":
            try:
                spks_rt = utils.zscore(
                    spks[:, np.asarray(trial != 0).nonzero()[0]], ax=1, epsilon=1e-5
                )
                spks_rt = np.array(
                    [
                        spks_rt[:, i] - np.min(spks_rt, axis=1)
                        for i in range(spks_rt.shape[1])
                    ]
                ).T
                if plotting:
                ...
                spks_t = np.zeros(
                    (spks.shape[0], 1 + np.max(np.asarray(trial != 0).nonzero()[0]))
                )
                spks_t[:, : spks.shape[1]] = spks
                spks_rt = utils.zscore(
                    spks_t[:, np.asarray(trial != 0).nonzero()[0]], ax=1, epsilon=1e-5
                )
...
def loadFluoMesoscope(
    ...
    if first:
        ...
        if Nb_plane != 1:
            ...
            if plane != -1:
                ...
                F = np.load(path + "/plane%d/F.npy" % plane)[
                    np.load(path + "/plane%d/iscell.npy" % plane)[:, 0].astype(bool)
                ][:, :block_end]

                Fneu = np.load(path + "/plane%d/Fneu.npy" % plane)[
                    np.load(path + "/plane%d/iscell.npy" % plane)[:, 0].astype(bool)
                ][:, :block_end]
                ...
                M = [
                    np.load(path + "/plane%d/F.npy" % p)[
                        np.load(path + "/plane%d/iscell.npy" % p)[:, 0].astype(bool)
                    ]
                    - (
                        0.7
                        * np.load(path + "/plane%d/Fneu.npy" % p)[
                            np.load(path + "/plane%d/iscell.npy" % p)[:, 0].astype(bool)
                        ]
                    )
                ...
                spks = np.concatenate(
                    [M[i][:, : M[-1].shape[1]] for i in range(len(M))]
                )[:, :block_end]
                    ...
                    + np.asarray(
                        [
                            ...
                            for sta in np.load(
                                ...
                                np.load(path + "/plane%d/iscell.npy" % plane)[
                                    :, 0
                                ].astype(bool)
                            ]
                        ]
                    ...
                    + np.asarray(
                        [
                            ...
                            for sta in np.load(
                                ...
                                np.load(path + "/plane%d/iscell.npy" % p)[:, 0].astype(
                                    bool
                                )
                            ]
                        ]
                    )
                    for p in range(1, Nb_plane)
                ]
            )
    ...
    resps_all, resps_all2 = align_datas(
        exp_info,
        dirs,
        spks,
        Nb_frames,
...
def loadSPKMesoscope(
    ...
    if first:
        ...
        if Nb_plane != 1:
            ...
            if plane != -1:
                print("loading planes nb ", plane)
                spks = np.load(path + "/plane%d/spks.npy" % plane)[
                    np.load(path + "/plane%d/iscell.npy" % plane)[:, 0].astype(bool)
                ][:, :block_end]
                ...
                print("loading all planes")
                M = [
                    np.load(path + "/plane%d/spks.npy" % p)[
                        np.load(path + "/plane%d/iscell.npy" % p)[:, 0].astype(bool)
                    ]
                    for p in range(Nb_plane)
                ]
...
def loadRespMesoscope(exp_info, dirs, path, block_end):
    ...
    spks_deconvolved = np.concatenate(
        [
            np.load(path + "/plane%d/spks.npy" % plane)[
                np.load(path + "/plane%d/iscell.npy" % plane)[:, 0].astype(bool)
            ]
    ...
    F = np.concatenate(
        [
            np.load(path + "/plane%d/F.npy" % plane)[
                np.load(path + "/plane%d/iscell.npy" % plane)[:, 0].astype(bool)
            ]
            for plane in range(3)
        ]
    ...
    Fneu = np.concatenate(
        [
            np.load(path + "/plane%d/Fneu.npy" % plane)[
                np.load(path + "/plane%d/iscell.npy" % plane)[:, 0].astype(bool)
            ]
            for plane in range(3)
        ]
    ...
    trial1 = np.logical_and(
        frame_times >= syncEcho_flip_times[0],
        frame_times < syncEcho_flip_times[Nb_frames],
    )
    time_trial1 = frame_times[trial1]

    trial2 = np.logical_and(
        frame_times >= syncEcho_flip_times[Nb_frames],
        frame_times < syncEcho_flip_times[Nb_frames * 2],
    )
    time_trial2 = frame_times[trial2]

    trial3 = np.logical_and(
        frame_times >= syncEcho_flip_times[Nb_frames * 2],
        frame_times < syncEcho_flip_times[Nb_frames * 3],
    )
    time_trial3 = frame_times[trial3]

    trial4 = np.logical_and(
        frame_times >= syncEcho_flip_times[Nb_frames * 3],
        frame_times < syncEcho_flip_times[Nb_frames * 4],
    )
    time_trial4 = frame_times[trial4]

    trial5 = np.logical_and(
        frame_times >= syncEcho_flip_times[Nb_frames * 4],
        frame_times < syncEcho_flip_times[Nb_frames * 5],
    )
    time_trial5 = frame_times[trial5]

    trial6 = np.logical_and(
        frame_times >= syncEcho_flip_times[Nb_frames * 5],
        frame_times < syncEcho_flip_times[Nb_frames * 7],
    )
    time_trial6 = frame_times[trial6]
...
def get_rfs(i, r2, r1, i2, i1, y2, y1, neuron_pos):
    ...
    np.save(
        "/media/sophie/Expansion1/UCL/datatest/SP045/2023-10-04/3/rfs/rfs_%d_HR.npy"
        % i,
        RFS,
    )
...
def correctNeuronPos(neuron_pos, resolution=1.3671):
    ...
    neuron_pos[np.logical_and(neuron_pos[:, 0] > ly, neuron_pos[:, 0] <= 2 * ly)] = (
        neuron_pos[np.logical_and(neuron_pos[:, 0] > ly, neuron_pos[:, 0] <= 2 * ly)]
        + np.array([-ly, lx])
    )
    neuron_pos[neuron_pos[:, 0] > 2 * ly] = neuron_pos[
        neuron_pos[:, 0] > 2 * ly
    ] + np.array([-2 * ly, 2 * lx])
...
def coarseWavelet(
    path, downsampling, nx0=135, ny0=54, nx=27, ny=11, no=8, ns=6, nf=1, chunk_size=1000
):
    """
    ...
    if os.path.exists(os.path.join(path, "dwt_downsampled_videodata.npy")):
        ...
        wavelets_downsampled = np.load(
            os.path.join(path, "dwt_downsampled_videodata.npy")
        )
        ...
        for i in range(nb_chunks):
            ...
            w_c_downsampled = skimage.transform.resize(
                ...
                (wavelets_complex.shape[0], nx, ny, no, ns, nf),
                anti_aliasing=True,
            )
        ...
        np.save(
            os.path.join(path, "dwt_downsampled_videodata.npy"),
            [w_r_downsampled, w_i_downsampled, w_c_downsampled],
        )
